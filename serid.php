 ...
 <table border="0" bgcolor="blue" cellspacing="0">
   <tr>
	...
	<table>
			<tr>
				<td>
					<div><b>View ID information</b></div>
					<hr />
						<form action="serid.php" onsubmit='return formValidator()' method='POST'>
						<table align="center">
							<tr>
								<td class="search">Enter Your First Name:</td><td><input type="text" name="fname" size="40px" placeholder="Provide Here... "value="<?php echo ucfirst($_SESSION['username']); ?>" readonly="readonly" /></td>

<tr>							<td class="search">Enter House Number:</td><td><input type="text" name="houseno" size="40px" placeholder="Provide Here..." /></td>

<tr>
								<td class="search">Enter your Phone Number:</td>

								<td><input type="text" name="phone" size="40px" placeholder="Provide Here..." /></td>
					...
					if(isset($_POST['search']))
 {
					...
					$house = $_POST['houseno'];
					$phone = $_POST['phone'];
					$sql = "SELECT * FROM id_card WHERE fname=? AND houseno=? AND phone=?";
                    $stmt = mysqli_prepare($conn, $sql);
                    mysqli_stmt_bind_param($stmt, "sss", $idno, $house, $phone);
                    mysqli_stmt_execute($stmt);
                    $result = mysqli_stmt_get_result($stmt);
					$count = mysqli_num_rows($result);
					if($count < 1)
					{
					die('<font color="red">By this Information you have not id card, please enter correctil!?</font>');		
					}
					else
					{
					echo"<center>";
...
print ("<td>" . $row['age'] . "</td>");
   ...
   print ("<td>" . $row['dategiven'] . "</td>");
   print ("<td>" . $row['Kebele'] . "</td>");
   print ("<td>" . $row['approvedby'] . "</td>");
   print ("<td>" . $row['location'] . "</td>");
   print ("<td>" . $row['renewed_date'] . "</td>");
  print("<td align = 'center' width = '1'> <a href = 'viewid.php?key1=".$ctrl."'>
<img width='25px' height='25px' src = 'images/user.jpg' title='view ID card information'></a>
</td>	
